...
const Page = () => {
  ...
  const [pickUpSuggesions, setPickUpSuggestions] = useState<Place[]>([]);
  const [locating, setLocating] = useState<boolean>();
    ...
    navigator.geolocation.getCurrentPosition(async (c) => {
      try {
        const { data } = await axios.get(
          `https://photon.komoot.io/reverse?lon=${c.coords.longitude}&lat=${c.coords.latitude}`,
        );
        ...
        setPickUpLat(c.coords.latitude);
        setPickUpLon(c.coords.longitude);
        setLocating(false);
      } catch (err) {
        console.log(err);
        setLocating(false);
  ...
  const searchAddress = async (
    p: any,
    setPickUpSuggestion: (r: Place[]) => void,
    ...
    try {
      ...
      const { data } = await axios.get(
        `https://photon.komoot.io/api/?q=${encodeURIComponent(p)}&limit=8&lang=en`,
      );
